...
export class R2Error extends TaggedError("R2Error")<{
  ...
  constructor(args: { reason: R2Reason; cause: unknown }) {
    const message = R2Error.#formatMessage(args.reason);
    super({ ...args, message });
  }

  static #formatMessage(reason: R2Reason): string {
    switch (reason.op) {
      case "head":
        return `Failed to get metadata for "${reason.key}"`;
      case "get":
        return `Failed to get "${reason.key}"`;
      case "put":
        return `Failed to put "${reason.key}"`;
      case "delete": {
        const keyStr = Array.isArray(reason.keys)
          ? reason.keys.join(", ")
          : reason.keys;
        return `Failed to delete "${keyStr}"`;
      }
      case "list":
